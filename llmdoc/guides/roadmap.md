...
┌─────────────┐         ┌─────────────┐         ┌─────────────┐         ┌─────────────┐
│ Tauri 骨架   │         │ VLM 屏幕理解 │         │ OpenAI API  │         │ WASM 插件   │
...
- [x] 完整的屏幕理解与文本提取功能 (通过 VLM)
- [x] 支持自然语言的语义搜索
- [x] 搜索结果可视化与高级过滤

---

## Phase 3: 记忆合成 (The Mind) - 100% 完成
...
| 任务   | 描述                     | 产出                          | 状态 |
| ------ | ------------------------ | ----------------------------- | ---- |
| T3.1.1 | 实现 LLM HTTP API 客户端 | OpenAI 兼容的摘要生成能力     | ✅   |
| T3.1.2 | 实现摘要 Prompt 模板     | 结构化摘要输出                | ✅   |
| T3.1.3 | 实现实体提取 Prompt      | 从摘要中提取实体              | ✅   |

### M3.2: 周期摘要

| 任务   | 描述             | 产出         | 状态 |
| ------ | ---------------- | ------------ | ---- |
| T3.2.1 | 15 分钟摘要任务  | 短周期摘要   | ✅   |
| T3.2.2 | 每日摘要聚合     | 长周期摘要   | ✅   |
| T3.2.3 | summaries 表存储 | 摘要持久化   | ✅   |
...
| **M2** | 输入"昨天看的 Rust 文章"能找到相关截图           | ✅ 100%                  |
| **M3** | 自动生成"今日工作摘要"并提取提到的人名/项目      | ✅ 100%                  |
...
└── src/
    ...
    │   └── summarizer_task.rs # SummarizerTask 定时摘要
    ├── ai/                    # AI 模块
    │   ├── mod.rs             # 模块导出
    │   ├── vlm.rs             # VLM 屏幕理解
    │   ├── embedding.rs       # 文本嵌入
