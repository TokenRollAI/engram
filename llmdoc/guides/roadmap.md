...
| 任务 | 描述 | 当前状态 |
|------|-----|---------|
| T1.2.3 | WebP 压缩存储 | 当前使用 PNG，WebP 编码待实现 |
| T1.3.1 | 窗口信息获取 | 结构已定义，平台实现待完成 |

### 待开始 📋

| 任务 | 描述 | 依赖 |
|------|-----|------|
| T1.3.2 | 闲置检测 | 需集成 user-idle-time crate |
...
| 任务 | 描述 | 产出 |
|------|-----|------|
| T2.1.1 | 集成 ort (ONNX Runtime) | 推理框架基础设施 |
| T2.1.2 | 实现 PaddleOCR 检测管道 | 文本区域检测 |
| T2.1.3 | 实现 PaddleOCR 识别管道 | 文本内容识别 |
| T2.1.4 | 优化各平台执行提供者 | CoreML/DirectML/CUDA |
...
| 里程碑 | 验收标准 | 状态 |
|--------|---------|------|
| **M1** | 应用能后台运行，截取屏幕并存储，可浏览历史 | 🚧 82% |
| **M2** | 输入"昨天看的 Rust 文章"能找到相关截图 | 📋 0% |
